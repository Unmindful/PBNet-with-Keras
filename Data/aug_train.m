...
dataDir = 'Train Data/291';
folder=fullfile('Train Data','train_291_128x128bicY');
mkdir(folder);
...
for f_iter = 1:numel(f_lst)
    ...
    for x = 0:x_size-1
        for y = 0:y_size-1
            ...
            save(sprintf('%s_4', patch_name), 'patch');
            patch = imrotate(img_8(y_coord+1:y_coord+patch_size,x_coord+1:x_coord+patch_size,:), 90);
            save(sprintf('%s_8', patch_name), 'patch');
            
            count = count+1;
            
            % Flipped Alignment of Actual image
            ...
            save(sprintf('%s_4', patch_name), 'patch');
            patch = fliplr(imrotate(img_8(y_coord+1:y_coord+patch_size,x_coord+1:x_coord+patch_size,:), 0));
            save(sprintf('%s_8', patch_name), 'patch');
            
            count = count+1;
            
             % 90 degree Alignment of Flipped Actual image
            ...
            save(sprintf('%s_4', patch_name), 'patch');
            patch = fliplr(imrotate(img_8(y_coord+1:y_coord+patch_size,x_coord+1:x_coord+patch_size,:), 90));
            save(sprintf('%s_8', patch_name), 'patch');
